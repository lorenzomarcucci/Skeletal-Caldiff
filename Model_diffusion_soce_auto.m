...
n=20; %sections in radial direction
m=50; %sections in longitudinal direction
D=3*1e2; %microm^2s-1
t_tot=120;
...
for i_soce=2:2
    ...
    for anls=1:1
        ...
        %Ca_tot=1.2*1e-9*V_max; %Total Calcium in mM/liter*V_max
        %SR permeability as function of time
        ...
        for freq=1:length(hertz)
            ...
            clear x_s t_s x0
            tic
            % filename=['HQ_' filename];
            
            % Diffusional rate constants and time
            t_start=0;
            t_finish=held(freq)+t_tot; %total time of the analysis
               ...
               % tspan = linspace(t_ini, t_end, t_points);
                ...
                if t_ini>0
                    if sampling_switch==1
                        ...
                        %plot(t_s,x_s(:,57)+x_s(:,58)-x_s(1,58),'g')
                        %plot(t_s,mean(x_s(:,1:m*(n-1)),2),'r')
                        % plot(t_s,mean(x_s(:,m*(n-1)+1:m*n),2),'r')
                        plot(t_s,x_s(:,Mito_free)/1.6,'g.',...
                            t_s,mean(x_s(:,1:m*(n-1)),2),'r')
                        %plot(t_s,(x_s(:,Mito_free)))
                        t_e=length(t_s);
                        t_i=find(t_s>=t_ini-t_step+t_step/10,1);
                        ...
                        mid=mean(p)
                        %plot(t_s,x_s(:,57),'r')
                        %plot(t_s,x_s(:,m*n+9)*V_EF)
                        %plot(t_s,x_s(:,31),'r')
                        %plot(t_s,x_s(:,(m*(n))+7),'k')
                    end
...
%plot(t_s,(x_s(:,Tr2_b+M_Tr_index)*volumes(M_Tr_index)')/V_max/120,'linewidth',5)
%pause(1)


save last

plot(t_s,(x_s(:,1:Mg_b)*volumes'...
    +x_s(:,CaS)*V_TC+x_s(:,CaS2)*V_TC...
    +x_s(:,CaPV_b+1:MgPV_b)*volumes(1:m*(n-1))'...
    +x_s(:,Fura_b+1:Tr1_b)*volumes(1:m*(n-1))'...
    +x_s(:,Tr1_b+1:Tr2_b)*volumes(1:m*(n-2))'...
    +x_s(:,Tr2_b+1:CaS_b)*volumes(1:m*(n-2))'...
    +x_s(:,Tr2_b+1:CaS_b)*volumes(1:m*(n-2))'...
    +(x_s(:,Mito_free)+x_s(:,Mito_bound1)+x_s(:,Mito_bound2))*V_mito...
    +x_s(:,Ttubule_free)*V_tub+x_s(:,Ttubule_bound)*V_tub)/V_max,'linewidth',3)

%plot(t_s,(x_s(:,1:(m*n))*volumes')/V_max)


%plot(t_s,x_s(:,52)/max(x_s(:,52)),'-r',t_s,x_s(:,47)/max(x_s(:,47)),t_s,x_s(:,37)/max(x_s(:,37)),t_s,x_s(:,51)/max(x_s(:,51)))


%plot(t_s,(x_s(:,m*(n-1)+1:m*n)*volumes(m*(n-1)+1:m*n)'/V_max))
...
prompt = 'Compartment? [1_cito, 2_RS 3_Mitofree 4_MitoAll 5_CSQ 6PVCa] ';
x = input(prompt)

if x==1
    plot(t_s,mean(x_s(:,1:m*(n-1)),2))
elseif x==2
    plot(t_s,mean(x_s(:,m*(n-1)+1:m*n),2))
elseif x==3
    plot(t_s,x_s(:,Mito_free),'linewidth',3)
elseif x==4
    plot(t_s,(x_s(:,Mito_free)+x_s(:,Mito_bound1)+x_s(:,Mito_bound2))*V_mito/V_max,'linewidth',3)
elseif x==5
    plot(t_s,(x_s(:,CaS)*V_TC+x_s(:,CaS2)*V_TC)/V_max,'linewidth',3)
elseif x==6
    plot(t_s,(x_s(:,CaPV_b+1:MgPV_b)*volumes(1:m*(n-1))')/V_max,'linewidth',3)
end


end
plot(t_s,(x_s(:,Tr2_b+1:CaS_b)*volumes(1:m*(n-2))')/V_max/120,'linewidth',3)
plot(t_s,(x_s(:,Ttubule_free)*V_tub+x_s(:,Ttubule_bound)*V_tub)/V_max,'linewidth',3)
plot(t_s,(x_s(:,Mg_b+1:CaPV_b)*volumes(1:m*(n-1))')/V_max,'linewidth',3)
plot(t_s,(x_s(:,Fura_b+1:Tr1_b)*volumes(1:m*(n-1))')/V_max,'linewidth',3)

p=plot(t_s,(x_s(:,1:m*(n-1))*volumes(1:m*(n-1))'/V_max),'r',...
    t_s,(x_s(:,m*(n-1)+1:m*n)*volumes(m*(n-1)+1:m*n)'+...
    x_s(:,CaS)*V_TC+x_s(:,CaS2)*V_TC)/V_max,'b',...
    t_s,(x_s(:,CaPV_b+1:MgPV_b)*volumes(1:m*(n-1))')/V_max,'k',...
    t_s,(x_s(:,Fura_b+1:Tr1_b)*volumes(1:m*(n-1))')/V_max,'y',...
    t_s,(x_s(:,Tr1_b+1:Tr2_b)*volumes(1:m*(n-2))'+...
    x_s(:,Tr2_b+1:CaS_b)*volumes(1:m*(n-2))'+...
    x_s(:,Tr2_b+1:CaS_b)*volumes(1:m*(n-2))')/V_max,'c',...
    t_s,(x_s(:,Mito_free)+x_s(:,Mito_bound1)+x_s(:,Mito_bound2))*V_mito/V_max,'m',...
    t_s,(x_s(:,Ttubule_free)*V_tub+x_s(:,Ttubule_bound)*V_tub)/V_max,'g');
...
%{
%force and thin filament activation from paper/homogeneous/steady_state/pCa/
V=[0.10000E-01      0.30739E+03      0.15968E-01,
0.18738E-01      0.71016E+03      0.30943E-01,
0.35112E-01      0.29308E+04      0.58013E-01,
0.65793E-01      0.28346E+05      0.11540E+00,
0.12328E+00      0.17157E+06      0.22278E+00,
0.23101E+00      0.42271E+06      0.39033E+00,
0.43288E+00      0.73209E+06      0.59588E+00,
0.81113E+00      0.97592E+06      0.77166E+00,
0.15199E+01      0.11056E+07      0.88106E+00,
0.28480E+01      0.11700E+07      0.93998E+00,
0.53367E+01      0.12009E+07      0.96882E+00,
0.10000E+02      0.12156E+07      0.98393E+00];
plot(V(:,3),V(:,2)/V(end,2),'o')
xlabel('Actin activated ratio')
ylabel('T/T0')


for i_n=1:n-1
for i_m=1:m
z(i_m,i_n)=x_s(end,i_m+(i_n-1)*m);
end
end
%[X,Y]=meshgrid(0:R_max/(n-2):R_max,0:L_max/(m-1):L_max);
surf(z)
xlabel('Radial','Rotation',30)
ylabel('Longitudinal','Rotation',-35)
xticks([1 n-1])
xticklabels({'Center','SR'})
yticks([1 m])
yticklabels({'M-Line','Z-Line'})
zlabel('Ca^{2+} (µM)','Rotation',90)
%}